...
| P2 | 前端协议选择器 + 状态流 + 蒙版几何 + 文档 | `37520f4` |
| C2 | 跨域下载修复 + 大图灯箱 | `83cfc2d` |
| C3 | 蒙版橡皮擦/撤销/画笔光标/拖拽/等比画布 | `ae2b089` |
...
## ⏳ 阶段三 · C3：RegionEditor 蒙版分辨率对齐 + 橡皮擦/撤销 + 画笔光标

**问题背景**

- 蒙版当前以固定 **720×520 画布**导出，原图是 letterbox 居中绘制的；导出的 mask 是整张画布（含信箱黑边区域），**没有对齐到原图的像素网格**。多数 inpainting API（如 gpt-image）要求 mask 与原图同尺寸、像素对齐，当前实现存在正确性隐患。
- 缺少橡皮擦、撤销、画笔光标预览；不支持拖拽上传。
...
**验收标准**：导出 mask 尺寸 = 原图尺寸且区域与涂抹位置对齐；橡皮擦/撤销可用；画笔光标可见；拖拽上传可用；切换主题画布同步换色；`npm run build` 通过。
...
4. **文档**：更新 `README.md`、`docs/ARCHITECTURE.md`、`docs/API.md`，补充主题、历史、key 脱敏、蒙版对齐等新能力。
