...
- 局部编辑：上传/拖拽原图后直接在图上涂抹、框选、橡皮擦标记修改区域，半透明遮罩实时叠加；无需手动制作蒙版，前端自动合成混合图。支持撤销（Ctrl+Z）、遮罩显隐切换、整体放大编辑。
...
- 生成历史：左侧会话队列含实时状态标记，支持模糊搜索、时间筛选、失败重试覆盖、复用参数、回看结果。
- 设置：弹窗式，含 API 设置、偏好（最大字数 / 参考图上限）、提示词模板管理。
- 提示词：示例库（分类 + 随机）、放大编辑、字数外显、可配置上限（默认 3000）。
